...
export const fetchCartItems = createAsyncThunk('cart/fetchCartItems', async({userId}) => {
    ...
    const response = await  axios.get(`http://localhost:5000/api/shop/cart/${userId}`);
...
export const updateCartQuantity = createAsyncThunk('cart/updateCartQuantity', async({userId, productId, quantity}) => {
    ...
    const response = await  axios.put(`http://localhost:5000/api/shop/cart/update/${userId}/${productId}`, {
        quantity
    });
